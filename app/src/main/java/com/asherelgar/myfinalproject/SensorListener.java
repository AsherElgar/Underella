...
public class SensorListener extends Service implements SensorEventListener {
    ...
    private void updateIfNecessary() {
        ...
        DatabaseReference db = FirebaseDatabase.getInstance().getReference("Steps").child(user.getUid());
//        if (SensorListener.steps > lastSaveSteps + SAVE_OFFSET_STEPS ||
//                (SensorListener.steps > 0 && System.currentTimeMillis() > lastSaveTime + SAVE_OFFSET_TIME)) {
//            if (BuildConfig.DEBUG) Logger.log(
//                    "saving steps: steps=" + SensorListener.steps + " lastSave=" + lastSaveSteps +
//                            " lastSaveTime=" + new Date(lastSaveTime));
//            if (db.getSteps(Util.getToday()) == Integer.MIN_VALUE) {
//                int pauseDifference = SensorListener.steps -
//                        getSharedPreferences("pedometer", Context.MODE_PRIVATE)
//                                .getInt("pauseCount", SensorListener.steps);
//                db.insertNewDay(Util.getToday(), SensorListener.steps - pauseDifference);
//                if (pauseDifference > 0) {
//                    // update pauseCount for the new day
//                    getSharedPreferences("pedometer", Context.MODE_PRIVATE).edit()
//                            .putInt("pauseCount", SensorListener.steps).commit();
//                }
//            }
//            db.saveCurrentSteps(SensorListener.steps);
//            db.close();
//            lastSaveSteps = SensorListener.steps;
...
//        }
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        ...
        if (intent != null && ACTION_PAUSE.equals(intent.getStringExtra("action"))) {
            ...
            if (steps == 0) {
                String ref = db.getDatabase().getReference("steps").child(user.getUid()).getKey();
                Integer of = Integer.valueOf(ref);
                steps = of;
            }
...
//                                                .format((goal - today_offset - steps))));
            ...
            notificationBuilder.setPriority(Notification.PRIORITY_MIN).setShowWhen(false)
                    ...
                    .getActivity(this, 0, new Intent(this, StepCounterFragment.class),
                            PendingIntent.FLAG_UPDATE_CURRENT))
